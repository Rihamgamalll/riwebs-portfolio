...
export default function ProjectShowcase({lang}:{lang:Lang}){
  ...
  return <section className="work-x" id="work">
    ...
    <div className="work-x__intro"><h2 data-rise>{ar?'بعض المشاريع. وكل واحد عالم لوحده.':'Some projects. Each its own world.'}</h2></div>
    ...
    <div className="work-deck">
      {visible.map((p,i)=>{const palette=projectPalette[p.id]||{accent:'#8d6f7a',soft:'#eee7e4'};return <article className="work-card" data-tilt-scroll key={p.id} style={{'--card-index':i,'--project-accent':palette.accent,'--project-soft':palette.soft} as CSSProperties}>
